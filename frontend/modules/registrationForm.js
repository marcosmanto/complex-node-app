...
export default class RegistrationForm {
  ...
  events() {
    ...
    this.username.addEventListener('keyup', el => {
      // filter only keystrokes that represent changes in the typed string
      this.isDifferent(this.username, this.usernameHandler)
    })

    this.username.addEventListener('blur', el => {
      this.isDifferent(this.username, this.usernameHandler)
    })

    this.email.addEventListener('keyup', el => {
      this.isDifferent(this.email, this.emailHandler)
    })

    this.email.addEventListener('blur', el => {
      this.isDifferent(this.email, this.emailHandler)
    })

    this.password.addEventListener('keyup', el => {
      this.isDifferent(this.password, this.passwordHandler)
    })

    this.password.addEventListener('blur', el => {
      this.isDifferent(this.password, this.passwordHandler)
    })
  }
